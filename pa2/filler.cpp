...
animation filler::FillBFS(FillerConfig& config) {
  ...
  return filler::Fill<Queue>(config); // REPLACE THIS STUB
}
...
animation filler::FillDFS(FillerConfig& config) {
  ...
  // You should replace the following line with a
  // correct call to fill.
  return filler::Fill<Stack>(config); // REPLACE THIS STUB
}
...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
  ...
  */
  ...
  //       which pixels have already been visited
    ...
    for(int i = 0; (unsigned)i < config.img.width(); i++){
        processed.emplace_back();
        for(int j = 0; (unsigned) j < config.img.height(); j++){
            processed[i].push_back(0);
        }
    }

    config.neighbourorder = PriorityNeighbours(config.seedpoint.color);
    anim.addFrame(config.img);
    os.Add(config.seedpoint);
    ColorPicker * picker = config.picker; 

    if (config.seedpoint.color.dist(config.seedpoint.color) <= config.tolerance) {
       *(config.img.getPixel(config.seedpoint.x, config.seedpoint.y)) = picker->operator()(config.seedpoint);
    ...
    while(!os.IsEmpty()) {
        ...
        config.neighbourorder.SetReferenceColor(currPoint.color);
        int count = 0;

        if ((int) currPoint.x - 1 >= 0 && processed[currPoint.x-1][currPoint.y] == 0) {
            PixelPoint p = PixelPoint(currPoint.x-1, currPoint.y, *config.img.getPixel(currPoint.x-1, currPoint.y));
            if (p.color.dist(config.seedpoint.color) <= config.tolerance) {
              config.neighbourorder.Insert(p);
              count++;
            }
        }
        if ((int) currPoint.y + 1 < (int) config.img.height() && processed[currPoint.x][currPoint.y + 1] == 0) {
            PixelPoint p = PixelPoint(currPoint.x, currPoint.y+1, *config.img.getPixel(currPoint.x, currPoint.y+1));
            if (p.color.dist(config.seedpoint.color) <= config.tolerance) {
              config.neighbourorder.Insert(p);
              count++;
            };
        }
        if ((int) currPoint.x + 1 < (int) config.img.width() && processed[currPoint.x+1][currPoint.y] == 0) {
            PixelPoint p = PixelPoint(currPoint.x+1, currPoint.y, *config.img.getPixel(currPoint.x+1, currPoint.y));
            if (p.color.dist(config.seedpoint.color) <= config.tolerance) {
              config.neighbourorder.Insert(p);
              count++;
        ...
        if ((int) currPoint.y - 1 >= 0 && processed[currPoint.x][currPoint.y-1] == 0) {
            PixelPoint p = PixelPoint(currPoint.x, currPoint.y-1, *config.img.getPixel(currPoint.x, currPoint.y-1));
            if (p.color.dist(config.seedpoint.color) <= config.tolerance) {
              config.neighbourorder.Insert(p);
              count++;
        ...
        for (int i = 0; i < count; i++) {
          PixelPoint curr = config.neighbourorder.Remove();
          *(config.img.getPixel(curr.x, curr.y)) = picker->operator()(curr);
          os.Add(curr);
          processed[curr.x][curr.y] = 1;
          framecount++;
